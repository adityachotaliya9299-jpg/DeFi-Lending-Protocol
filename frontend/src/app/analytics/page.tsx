...
function genTvlData(days: number) {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    data.push({ date: d.toLocaleDateString("en-US", { month: "short", day: "numeric" }),
      TVL: Math.floor(tvl), Borrowed: Math.floor(tvl * (0.35 + Math.random() * 0.2)) });
  }
  return data;
}
function genApyData(days: number) {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    const d = new Date(); d.setDate(d.getDate() - i);
    data.push({ date: d.toLocaleDateString("en-US", { month: "short", day: "numeric" }),
      "WETH Supply": +Math.max(0.1,ws).toFixed(2), "WETH Borrow": +Math.max(0.5,wb).toFixed(2),
      "USDC Supply": +Math.max(0.1,us).toFixed(2), "USDC Borrow": +Math.max(0.5,ub).toFixed(2) });
  }
  return data;
}
...
function Tip({ active, payload, label, prefix="", suffix="" }: any) {
  ...
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)",
      ...
      <p style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-muted)", marginBottom: 7 }}>{label}</p>
      {payload.map((p: any) => (
        <div key={p.name} style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 3 }}>
          <div style={{ width: 8, height: 8, borderRadius: 2, background: p.color }} />
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-secondary)" }}>{p.name}:</span>
...
function RangePicker({ value, onChange }: { value: Range; onChange: (r: Range) => void }) {
  return (
    <div style={{ display: "inline-flex", background: "rgba(0,0,0,0.3)", border: "1px solid var(--border)",
      borderRadius: 10, padding: 3, gap: 3 }}>
      {(["7d","30d","90d"] as Range[]).map(r => (
        <button key={r} onClick={() => onChange(r)}
          style={{ padding: "5px 14px", borderRadius: 7, border: "none", cursor: "pointer",
            fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: value===r?700:400,
            background: value===r ? "var(--cyan)" : "transparent",
            color: value===r ? "#030712" : "var(--text-muted)", transition: "all 0.12s" }}>
          {r}
        </button>
      ))}
    </div>
...
function ChartCard({ title, subtitle, badge, live, children }: {
  title: string; subtitle?: string; badge?: string; live?: boolean; children: React.ReactNode;
}) {
  return (
      ...
      <div style={{ padding: "18px 22px", borderBottom: "1px solid var(--border)",
        ...
        <div>
          {subtitle && <p style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--cyan)",
            textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 3 }}>{subtitle}</p>}
          <p style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 15, color: "var(--text-primary)" }}>{title}</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {live && <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <span className="pulse-dot" />
            <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "#34d399" }}>Live</span>
          </div>}
          {badge && <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--text-muted)",
            background: "rgba(0,0,0,0.2)", border: "1px solid var(--border)", borderRadius: 6, padding: "3px 8px" }}>
            {badge}
          </span>}
        </div>
      </div>
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const snapshots = ((proto.data as any)?.dailyProtocolSnapshots ?? []) as DailyProtocolSnapshot[];
      setTvlData(snapshots.map(s => ({
      ...
      const wethSnaps = ((wethM.data as any)?.dailyMarketSnapshots ?? []) as DailyMarketSnapshot[];
      const usdcSnaps = ((usdcM.data as any)?.dailyMarketSnapshots ?? []) as DailyMarketSnapshot[];
      ...
      for (let i = 1; i < wethSnaps.length; i++) {
        ...
        const usdcApy = usdcSnaps[i]
          ? computeApy(usdcSnaps[i-1]?.liquidityIndex ?? "1", usdcSnaps[i].liquidityIndex, 1)
          : 0;
        apyRows.push({
      ...
      setVolData(wethSnaps.map((s, i) => ({
        ...
        USDC: usdcSnaps[i] ? Math.round(parseFloat(usdcSnaps[i].dailyBorrowVolume)) : 0,
        LINK: 0,
      })));

      // Liquidation chart
      const liqEvents = ((liqs.data as any)?.liquidations ?? []) as LiquidationEvent[];
      ...
      // Markets
      const mktData = ((mkts.data as any)?.markets ?? []) as MarketData[];
      setMarkets(mktData);

      // KPIs
      const latestSnap = snapshots[snapshots.length - 1];
      const totalLiqs  = liqEvents.length;
      const avgApy     = apyRows.length > 0
        ? (apyRows[apyRows.length-1]["WETH Supply"] + apyRows[apyRows.length-1]["USDC Supply"]) / 2
        : 0;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 md:px-6 py-10">
      ...
      <div className="reveal mb-8">
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", flexWrap:"wrap", gap:16 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            {loading && <span style={{ fontFamily:"var(--font-mono)", fontSize:11, color:"var(--text-muted)" }}>Loading…</span>}
            {liveData && <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              <span className="pulse-dot" />
              <span style={{ fontFamily:"var(--font-mono)", fontSize:11, color:"#34d399" }}>Subgraph live</span>
            </div>}
      ...
      {/* KPI row */}
      <div className="reveal reveal-delay-1" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, marginBottom:28 }}>
        {[
          { label:"Current TVL",       value: kpis.tvl,                    color:"var(--cyan)"  },
          { label:"Total Borrowed",    value: kpis.borrowed,               color:"#f87171"      },
          { label:`${range} Liquidations`, value: String(kpis.liquidations), color:"#f59e0b" },
          ...
          <div key={label} style={{ background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:14, padding:"16px 18px" }}>
            ...
            <p style={{ fontFamily:"var(--font-mono)", fontSize:24, fontWeight:500, color }}>{value || "—"}</p>
          </div>
        ))}
      </div>
      ...
      <div className="reveal" style={{ marginBottom:16 }}>
        <ChartCard title="Total Value Locked vs Borrowed" subtitle="TVL" live={liveData} badge={range}>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={tvlData}>
              ...
              <XAxis dataKey="date" tick={axisStyle} tickLine={false} axisLine={false} interval={interval}/>
              ...
              <Area type="monotone" dataKey="TVL"      stroke="#22d3ee" strokeWidth={2} fill="url(#gTvl)" dot={false}/>
              <Area type="monotone" dataKey="Borrowed" stroke="#f87171" strokeWidth={2} fill="url(#gBor)" dot={false}/>
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
        <div className="reveal">
          <ChartCard title="Supply APY" subtitle="Interest rates" live={liveData}>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={apyData}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)"/>
                <XAxis dataKey="date" tick={axisStyle} tickLine={false} axisLine={false} interval={interval}/>
                <YAxis tick={axisStyle} tickLine={false} axisLine={false} tickFormatter={v=>`${v}%`} width={38}/>
                <Tooltip content={<Tip suffix="%"/>}/>
                <Legend wrapperStyle={{ fontFamily:"var(--font-mono)", fontSize:11 }}/>
                <Line type="monotone" dataKey="WETH Supply" stroke="#22d3ee" strokeWidth={2} dot={false}/>
                <Line type="monotone" dataKey="USDC Supply" stroke="#a78bfa" strokeWidth={2} dot={false}/>
              </LineChart>
            </ResponsiveContainer>
          </ChartCard>
        </div>
        <div className="reveal reveal-delay-1">
          <ChartCard title="Borrow APY" subtitle="Interest rates" live={liveData}>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={apyData}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)"/>
                <XAxis dataKey="date" tick={axisStyle} tickLine={false} axisLine={false} interval={interval}/>
                ...
                <Tooltip content={<Tip suffix="%"/>}/>
                <Legend wrapperStyle={{ fontFamily:"var(--font-mono)", fontSize:11 }}/>
                <Line type="monotone" dataKey="WETH Borrow" stroke="#fbbf24" strokeWidth={2} dot={false}/>
                <Line type="monotone" dataKey="USDC Borrow" stroke="#f87171" strokeWidth={2} dot={false}/>
              </LineChart>
            </ResponsiveContainer>
          </ChartCard>
        </div>
      ...
      {/* Borrow volume */}
      <div className="reveal" style={{ marginBottom:16 }}>
        <ChartCard title="Daily Borrow Volume by Asset" subtitle="Volume" live={liveData} badge="Daily">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={volData} barSize={days===90?3:days===30?7:14}>
      ...
      <div className="reveal reveal-delay-1" style={{ marginBottom:16 }}>
        <ChartCard title="Liquidation History" subtitle="Liquidations" live={liveData} badge="Count">
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={liqData} barSize={days===90?3:days===30?7:14}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" vertical={false}/>
              <XAxis dataKey="date" tick={axisStyle} tickLine={false} axisLine={false} interval={interval}/>
              <YAxis tick={axisStyle} tickLine={false} axisLine={false} width={28}/>
              <Tooltip content={<Tip/>}/>
              <Bar dataKey="Count" fill="#ef4444" radius={[3,3,0,0]} fillOpacity={0.85}/>
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>
      ...
      {markets.length > 0 && (
        <div className="reveal">
          <p className="section-label" style={{ marginBottom:12 }}>Live market stats</p>
          <div style={{ background:"var(--bg-card)", border:"1px solid var(--border)", borderRadius:18, overflow:"hidden" }}>
              ...
              <table className="data-table" style={{ minWidth:600 }}>
                ...
                <tbody>
                  {markets.map(m => {
                    ...
                    const uColor = util>80?"#ef4444":util>60?"#f59e0b":"var(--cyan)";
                    return (
                      <tr key={m.id} style={{ borderBottom:"1px solid var(--border)" }}>
                        <td className="px-5 py-4" style={{ fontFamily:"var(--font-display)", fontWeight:700, color:"var(--text-primary)" }}>{m.symbol}</td>
                        <td className="px-5 py-4 text-right num text-sm" style={{ color:"var(--cyan)" }}>{fmtUsdBig(m.totalDepositUsd)}</td>
                        <td className="px-5 py-4 text-right num text-sm" style={{ color:"#f87171" }}>{fmtUsdBig(m.totalBorrowUsd)}</td>
                        <td className="px-5 py-4 text-right num text-sm" style={{ color:uColor }}>{util.toFixed(1)}%</td>
                        <td className="px-5 py-4 text-right num text-sm" style={{ color:"var(--text-secondary)" }}>{m.depositCount}</td>
                        <td className="px-5 py-4 text-right num text-sm" style={{ color:"var(--text-secondary)" }}>{m.borrowCount}</td>
                        <td className="px-5 py-4 text-right num text-sm" style={{ color:m.liquidationCount>0?"#f87171":"var(--text-muted)" }}>
                          {m.liquidationCount}
                        </td>
                      </tr>
                    );
