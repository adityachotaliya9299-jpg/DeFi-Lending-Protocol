...
contract CrossChainMessenger is AccessControl, ReentrancyGuard {
    ...
    event MessageSent(uint16 indexed dstChainId, address indexed user, uint8 msgType, address asset, uint256 amount, uint256 nonce);
    event MessageReceived(uint16 indexed srcChainId, address indexed user, uint8 msgType, address asset, uint256 amount, uint256 nonce);
    ...
    function setTrustedRemote(uint16 chainId, address remote) external onlyRole(ADMIN_ROLE) {
    ...
    function sendMessage(uint16 dstChainId, address user, uint8 msgType, address asset, uint256 amount)
        external nonReentrant
    {
        ...
        if (msgType < MSG_DEPOSIT || msgType > MSG_WITHDRAW) revert CCM__InvalidMsgType(msgType);

        uint256 nonce = nonces[dstChainId][user]++;
        emit MessageSent(dstChainId, user, msgType, asset, amount, nonce);
    }

    function lzReceive(uint16 srcChainId, address srcAddress, bytes calldata payload)
        external nonReentrant
    {
        if (trustedRemotes[srcChainId] != srcAddress)
            revert CCM__UntrustedRemote(srcChainId, srcAddress);

        (uint8 msgType, address user, address asset, uint256 amount, uint256 nonce) =
    ...
    function _processMessage(address user, uint8 msgType, address asset, uint256 amount) internal {
    ...
    function encodePayload(uint8 msgType, address user, address asset, uint256 amount, uint256 nonce)
        external pure returns (bytes memory)
    {
        return abi.encode(msgType, user, asset, amount, nonce);
    }

    function isTrustedRemote(uint16 chainId, address remote) external view returns (bool) {
        return trustedRemotes[chainId] == remote;
    }

    function getUserCrossChainPosition(address user, address asset)
        external view returns (uint256 deposited, uint256 borrowed)
    {
        return (crossChainDeposits[user][asset], crossChainBorrows[user][asset]);
    }
}
