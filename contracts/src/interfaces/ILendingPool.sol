...
interface ILendingPool {
    ...
    event IsolationConfigSet(
        address indexed asset,
        bool isolated,
        uint256 ceiling
    );
    event IsolationAllowedBorrowSet(
        address indexed collateral,
        address indexed borrowAsset,
        bool allowed
    );
    event EModeCategorySet(
        uint8 indexed id,
        uint16 ltv,
    ...
    error LendingPool__IsolationBorrowNotAllowed(
        address collateral,
        address borrowAsset
    );
    error LendingPool__IsolationDebtCeilingExceeded(
        address collateral,
        uint256 current,
        uint256 ceiling
    );
