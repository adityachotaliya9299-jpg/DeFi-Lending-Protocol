...
interface ICollateralManager {
    ...
    error CollateralManager__SupplyCapExceeded(address asset, uint256 cap, uint256 attempted);
    error CollateralManager__BorrowCapExceeded(address asset, uint256 cap, uint256 attempted);
    ...
    function checkSupplyCap(address asset, uint256 currentSupply, uint256 depositAmount) external view;
    function checkBorrowCap(address asset, uint256 currentBorrows, uint256 borrowAmount) external view;
    function calculateHealthFactor(
        address[] calldata collateralAssets,
        uint256[] calldata collateralUsds,
        uint256[] calldata debtUsds
    ) external view returns (uint256 healthFactor);
    function getMaxBorrow(address collateralAsset, uint256 collateralUsd) external view returns (uint256);
}
