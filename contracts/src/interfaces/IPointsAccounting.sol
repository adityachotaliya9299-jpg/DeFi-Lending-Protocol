...
interface IPointsAccounting {
    ...
    event PointsMinted(address indexed user, address indexed asset, uint256 amount, uint256 mode);
    event RateSet(address indexed asset, uint256 mode, uint256 rate);
    event PositionUpdated(address indexed user, address indexed asset, uint256 mode, uint256 newBalance, uint256 timestamp);
    ...
    function setAssetRate(address asset, uint256 supplyRate, uint256 borrowRate, bool active) external;

    function updatePosition(address user, address asset, uint256 mode, uint256 newBalance) external;
    ...
    function getPendingPoints(address user, address asset, uint256 mode) external view returns (uint256);

    function getUserPoints(address user) external view returns (uint256);

    function getPosition(address user, address asset, uint256 mode)
        external view returns (uint256 balance, uint256 lastUpdate, uint256 accruedPoints);
}
