...
contract StablecoinVault is ReentrancyGuard, Ownable {
    ...
    error Vault__DebtCeilingExceeded(
        address collateral,
        uint256 current,
        uint256 ceiling
    );
    ...
    function setCollateralConfig(
        ...
        if (cfg.collateralizationRatio <= cfg.liquidationRatio)
            revert Vault__InvalidConfig(
                "collateralRatio must be > liquidationRatio"
            );
    ...
    function mintPUSD(
        ...
        if (cfg.totalDebt + pUSDAmount > cfg.debtCeiling)
            revert Vault__DebtCeilingExceeded(
                collateral,
                cfg.totalDebt,
                cfg.debtCeiling
            );
        ...
        if (ratio < cfg.collateralizationRatio)
            revert Vault__BelowCollateralizationRatio(
                ratio,
    ...
    function depositAndMint(
        ...
        if (collateralAmount == 0 || pUSDAmount == 0)
    ...
    function liquidate(
        ...
        uint256 collToSeize = (seizeUsd * (10 ** collDecimals)) /
            collateralPrice;
    ...
    function getCollateralisationRatio(
        address user,
        address collateral
    ...
    function getPendingStabilityFee(
        address user,
        address collateral
        ...
        return
            (v.debtAmount * cfg.stabilityFeeBps * elapsed) /
            PRECISION /
            SECONDS_PER_YEAR;
    }
    ...
    function _getCollateralisationRatio(
        address user,
        address collateral
