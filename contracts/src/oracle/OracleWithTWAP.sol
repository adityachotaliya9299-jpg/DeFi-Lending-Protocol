...
contract OracleWithTWAP is IPriceOracle, Ownable {
    ...
    function registerFeed(
        ...
        _chainlinkFeeds[asset] = ChainlinkConfig(
            AggregatorV3Interface(feed),
            heartbeat
        );
     ...
     */
    function registerTwapFeed(
        address asset,
        address pool,
        bool assetIsToken0
    ) external onlyOwner {
        if (asset == address(0) || pool == address(0))
            revert PriceOracle__ZeroAddress();
        _twapFeeds[asset] = TwapConfig(
            IUniswapV3Pool(pool),
            assetIsToken0,
            true
        );
    ...
    function hasFeed(address asset) external view override returns (bool) {
        return
            _chainlinkFeeds[asset].heartbeat > 0 ||
            _twapFeeds[asset].registered;
    }

    // ── Internal — Chainlink ──────────────────────────────────────────────────

    function _tryGetChainlinkPrice(
        address asset
     ...
     */
    function _tryGetTwapPrice(
        address asset,
        TwapConfig memory tc
    ) internal view returns (bool ok, uint256 priceWad) {
    ...
    function _computeTwap(
        TwapConfig memory tc
    ) external view returns (uint256 priceWad) {
