...
contract RevenueDistributor is AccessControl, ReentrancyGuard {
    ...
    event RevenueClaimedStaker(
        address indexed user,
        uint256 indexed epoch,
        uint256 amount
    );
    event RevenueClaimedLP(
        address indexed user,
        uint256 indexed epoch,
        uint256 amount
    );
    ...
    function claimStaker(uint256 epoch) external nonReentrant {
        ...
        if (stakerClaimed[epoch][msg.sender])
            revert RevDist__AlreadyClaimed(msg.sender, epoch);
