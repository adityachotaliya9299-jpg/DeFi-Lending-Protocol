...
contract LocalDeploy is Script {
    function run() external {
        ...
        console2.log(
            "NEXT_PUBLIC_LWETH=",
            pool.getReserveData(address(weth)).lTokenAddress
        );
        console2.log(
            "NEXT_PUBLIC_LWBTC=",
            pool.getReserveData(address(wbtc)).lTokenAddress
        );
        console2.log(
            "NEXT_PUBLIC_LUSDC=",
            pool.getReserveData(address(usdc)).lTokenAddress
        );
        console2.log(
            "NEXT_PUBLIC_LLINK=",
            pool.getReserveData(address(link)).lTokenAddress
        );
    }
}
