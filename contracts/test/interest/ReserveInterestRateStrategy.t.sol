...
contract ReserveInterestRateStrategyTest is Test {
    ...
    function test_deployment_zeroOptimalReverts() public {
        vm.expectRevert(ReserveInterestRateStrategy.Strategy__OptimalUtilizationZero.selector);
        new ReserveInterestRateStrategy(admin, 0, BASE, SLOPE1_VAR, SLOPE2_VAR, SLOPE1_STB, SLOPE2_STB);
    }
    ...
    function test_rates_zeroUtilization() public view {
        (uint256 liqRate, uint256 stableRate, uint256 varRate) =
            strategy.calculateInterestRates(100_000e6, 0, 0, 0, RESERVE_FACTOR);
    ...
    function test_rates_aboveOptimal_steepSlope() public view {
        ...
        (, , uint256 varRate50) =
            strategy.calculateInterestRates(50_000e6, 0, 50_000e6, 0, RESERVE_FACTOR);
    ...
    function test_rates_liquidityRateLteVariableRate() public view {
        ...
        assertLt(liqRate, varRate, "supply rate < borrow rate due to reserve factor");
    }
    ...
    function test_updateStrategy_onlyAdmin() public {
        ...
        vm.expectRevert();
        strategy.updateStrategy(OPTIMAL, BASE, SLOPE1_VAR, SLOPE2_VAR, SLOPE1_STB, SLOPE2_STB);
    }
    ...
    function testFuzz_rates_variableRateMonotonicInUtilization(
        ...
        (, , uint256 rate1) = strategy.calculateInterestRates(avail1, 0, borr1, 0, RESERVE_FACTOR);
