...
contract MockOracleForPF {
    ...
    function getValueInUsd(address asset, uint256 amount) external view returns (uint256) {
...
contract MockCMForPF {
    ...
    function getAssetConfig(address asset) external view returns (
        ...
        return (c.ltv, c.liqThreshold, c.liqBonus, c.reserveFactor, c.supplyCap, c.borrowCap, c.isActive, c.isBorrowEnabled);
    }
}
...
contract LiquidationPathFinderTest is Test {
    ...
    function setUp() public {
        ...
        finder = new LiquidationPathFinder(admin, address(pool), address(oracle), address(cm));
    ...
    function test_findBestPath_returnsPath() public {
        LiquidationPathFinder.LiquidationPath memory best =
    ...
    function test_findBestPath_prefersHigherBonus() public {
        ...
        assertEq(best.collateralAsset, weth, "WETH wins due to larger USD value");
    ...
    function test_findBestPath_healthyPositionReverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            LiquidationPathFinder.PathFinder__HealthFactorOk.selector, borrower, 1.5e18
        ));
        finder.findBestPath(borrower, _colAssets(), _debtAssets());
    }
    ...
    function test_findBestPath_noValidPath_reverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            LiquidationPathFinder.PathFinder__NoLiquidatablePath.selector, emptyBorrower
        ));
    ...
    function test_findBestPath_seizeableIncludesBonus() public {
        ...
        uint256 expectedSeizable = (best.maxRepayUsd * (10_000 + best.bonusBps)) / 10_000;
        // Cap at collateral USD
        if (expectedSeizable > best.collateralUsd) expectedSeizable = best.collateralUsd;
        assertApproxEqAbs(best.seizeableUsd, expectedSeizable, 1e15);
    }

    function test_findBestPath_emitsEvent() public {
        vm.expectEmit(true, false, false, false);
        emit LiquidationPathFinder.PathFound(borrower, address(0), address(0), 0, 0);
        finder.findBestPath(borrower, _colAssets(), _debtAssets());
    }
    ...
    function test_rankPaths_returnsAllPaths() public view {
        (, uint256 count) = finder.rankPaths(borrower, _colAssets(), _debtAssets());
        // 2 collaterals x 2 debts = 4 paths (minus zero-balance paths)
        assertGt(count, 0);
    }

    function test_rankPaths_sortedByScoreDesc() public view {
        (LiquidationPathFinder.LiquidationPath[] memory paths, uint256 count) =
    ...
    function test_findBestPath_singleCollateral() public {
        ...
        col[0] = weth; dbt[0] = usdc;

        LiquidationPathFinder.LiquidationPath memory best =
            finder.findBestPath(borrower, col, dbt);
