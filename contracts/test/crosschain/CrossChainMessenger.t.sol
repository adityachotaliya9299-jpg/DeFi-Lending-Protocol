...
contract CrossChainMessengerTest is Test {
    ...
    function test_sendMessage_emitsEvent() public {
        ...
        emit CrossChainMessenger.MessageSent(REMOTE_CHAIN, alice, 1, asset, 1_000e6, 0);
    ...
    function test_sendMessage_invalidMsgTypeReverts() public {
        vm.expectRevert(abi.encodeWithSelector(CrossChainMessenger.CCM__InvalidMsgType.selector, 9));
    ...
    function test_lzReceive_withdrawReducesBalance() public {
        // First deposit
        bytes memory depositPayload = local.encodePayload(1, alice, asset, 1_000e6, 0);
        remote.lzReceive(LOCAL_CHAIN, address(local), depositPayload);

        // Then withdraw
        bytes memory withdrawPayload = local.encodePayload(4, alice, asset, 400e6, 1);
    ...
    function test_lzReceive_replayAttackPrevented() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            CrossChainMessenger.CCM__InvalidNonce.selector, 1, 0
        ));
        remote.lzReceive(LOCAL_CHAIN, address(local), payload);
    }
    ...
    function test_lzReceive_untrustedRemoteReverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            CrossChainMessenger.CCM__UntrustedRemote.selector, LOCAL_CHAIN, untrusted
        ));
