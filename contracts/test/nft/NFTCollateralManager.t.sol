...
contract NFTCollateralManagerTest is Test {
    ...
    function test_addCollection_ltvTooHighReverts() public {
        vm.prank(admin);
        vm.expectRevert(NFTCollateralManager.NFTCollateral__LtvTooHigh.selector);
    ...
    function test_updateFloorPrice_zeroReverts() public {
        vm.prank(admin);
        vm.expectRevert(NFTCollateralManager.NFTCollateral__ZeroFloorPrice.selector);
    ...
    function test_depositNFT_tracksPosition() public {
        _deposit(alice, 1);
        NFTCollateralManager.NFTPosition memory pos = ncm.getPosition(address(nft), 1);
    ...
    function test_depositNFT_unsupportedCollectionReverts() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            NFTCollateralManager.NFTCollateral__CollectionNotSupported.selector,
            address(unsupported)
        ));
    ...
    function test_withdrawNFT_withActiveLoanReverts() public {
        ...
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(
            NFTCollateralManager.NFTCollateral__HasActiveLoan.selector, address(nft), 1
        ));
        ncm.withdrawNFT(address(nft), 1);
    }
    ...
    function test_withdrawNFT_notOwnerReverts() public {
        ...
        vm.prank(bob);
        vm.expectRevert(abi.encodeWithSelector(
            NFTCollateralManager.NFTCollateral__NotOwner.selector, bob, alice
        ));
        ncm.withdrawNFT(address(nft), 1);
    }
